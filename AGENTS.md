...
| 15 | `_GridProcess()` :178 | ReadYaml + UnstMesh | `localFlex`, cached FEM operator/KSP (flat `fem`) or DH grid (`global`); regular grid only if orography on |
...
- `_readFlex` → `flexure.method` is `'fem'` (flat, default) or `'global'` (spherical); `'FD'`/`'FFT'`/gFlex were removed. `thick`/`rhoc`/`rhoa`/`young`/`nu`; `ninterp` is global-only (DH-grid KDTree). `regdx` is no longer a flexure key (only orography uses the regular grid).
...
| 2026-06-18 | — (branch `feat/cyclic-boundaries`) | **Cyclic (periodic) flow/sediment boundaries — `bc` char `'c'`.** `domain.bc` is now a 4-char string [S,E,N,W] over `{'0' open, '1' closed, 'c' cyclic}` (was int 0/1). `unstructuredmesh` maps them to `self.{south,east,north,west}` ∈ {0,1,2}; the open-edge sentinel treatment in `applyForces` keys on `==0`, so closed(1) AND cyclic(2) both skip it. Parser (`_readDomain`) validates: cyclic must be an OPPOSITE PAIR (S+N or E+W) and AT MOST ONE pair (≤2 periodic edges, never a 4-edge torus → would be mis-detected as global). **No routing code change**: cyclic relies on a PERIODIC INPUT MESH — specifically a CYLINDER (periodic axis wrapped onto a circle of circumference = domain width). A cylinder is intrinsically flat, so `definetin`'s FV geometry equals a periodic strip's and the wrap cells link the two seam edges in `FVmesh_ngbID` → flow/sediment route across the seam (works in parallel; DMPlex partitions the cylinder). The cylinder's two ends are the real (non-periodic) boundary, so `flatModel` stays True. A naive PLANAR wrap does NOT work (seam cells get domain-spanning geometry). Fixture `cyclic_cyl.npz` (1008-node cylinder) + `test_cyclic_boundary` (parse flags, cross-seam FV links, seam-not-drained, runs+finite). Docs: `user_guide/inputfile.rst` `bc`. Suite 75/1. |
