...
Opt-in via an `ice` YAML section (`self.iceOn`). **`ice.flow_model`** selects the ice driver: `sia` (default — the explicit SIA thickness solve, below) or `mfd` (a cheap diagnostic — `_iceFlowMFD`). Both feed the SAME abrasion / till / loading / output machinery (`iceUbL`→`iceAbrL`→`glacialTill`, `iceMeltL`, flexure); only the thickness/velocity *source* differs. Full design: `docs/DESIGN_ICE_SHEET.md`; change log: `docs/ICE_SHEET_SUMMARY.md`. Lives in `flow/iceplex.py` (`IceMesh`).

**Diagnostic (`flow_model: mfd`).** `_iceFlowMFD` routes the ELA accumulation (`mdot⁺·larea`, a volume rate) downhill through an MFD matrix (`_matrixIceFlow`, same epsfill+`mfdreceivers` machinery as water) into an **ice discharge** `iceFAL`/`iceFAG` (m³/yr, one `_solve_KSP`), smooths it, derives a **Bahr thickness** `iceHL = icewe·icewf·Q^0.3` and a **balance velocity** `iceUbL = Q/(H·√area)` driving `E_g=Kg|u_b|^l`. NO dynamics solve — ~1 routing solve/step, stable + physical at any resolution (real 30 km global: 0.8 s/step, `Hmax`~1 km), where the SIA becomes stiff/over-thick for km continental ice. Config: `icedir`/`eheight`(icewe)/`fwidth`(icewf)/`melt`(meltfac). New persistent Vecs `iceFAL`/`iceFAG` + cached `iceMat` are in `destroy_DMPlex`. Output adds `iceFA`.
