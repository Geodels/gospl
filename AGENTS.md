...
- **`self.gid` is ALSO partition-dependent** (PETSc's per-partition DMPlex numbering — `gid[locID]` differs across np, identity only at np=1). So the `gid` tie-break in `mfdreceivers` is *technically* non-invariant too; it survives only because strict-downhill exact-slope ties are rare. The partition-invariant physical id is **`self.locIDs`** (input-mesh index via KDTree, what the analysis tools also use).
