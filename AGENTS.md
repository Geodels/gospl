...
|---|---|---|---|
| `flow/flowplex.py` | `_solve_KSP` | `self._ksp_main` | KSP, **`fgmres`+`bjacobi`** (`flowacc_` prefix, `pc_factor_shift_type nonzero`), `max_it=5000`. Solves the IDA `(I−Wᵀ)q=b` flow/sediment-routing system. See **`## Flow-accumulation KSP`** below for why fgmres (not the old stationary richardson) and the seed/fatal knobs. Env overrides `GOSPL_FLOW_KSP` / `GOSPL_FLOW_PC`. |
| `flow/flowplex.py` | `_solve_KSP2` | `self._ksp_fallback` | KSP, **`richardson`+`none`** bounded fallback (`flowaccfb_` prefix, `max_it=2000`). Reached only when the primary fails on a (near-)singular sub-region; confirms non-convergence cheaply then degrades gracefully (zeroed, finite). |
...
- **On primary failure: never accept the iterate.** It restarts the fallback from a clean BOUNDED guess (the seed `b`, else a finite warm iterate) and lets the bounded fallback finish (zeroes a non-convergent solve). *Do not* "accept fgmres best-effort on max_it" — on a near-singular sub-region the **residual** is small but the **solution** is unbounded (null-space blowup), so accepting injects huge values that compound through the cascade (observed 1e29 escalation). The `[flowKSP] worst residual … N nodes` diagnostic prints on any fallback failure.

**Mechanism #2 of the partition-dependence (FIXED — was the residual drainage non-invariance).** The cascade solves run on the **filled** topography. A local A/B harness (run the fill + flat-routing at np=1 vs np=N on a 370k earth mesh, gather `lFill`/`lsink`/flat-receivers to rank 0 in **`locIDs`** input-mesh order, diff) isolated it precisely:
- The **fill (`lFill`), the sink set (`lsink`), and the BFS distance field (`ptdir`) are all bit-for-bit partition-invariant.** Not the source (my earlier note wrongly implicated `spill_pts`/`label_pits`/the BFS broadly).
- The **flat-routing receiver choice was NOT.** In a flat region many neighbours tie at the minimum `ptdir`, and the fortran `fill_rcvs` broke that tie by *"first in `FVnID` neighbour order"* — partition-DEPENDENT (local numbering / halo). So ~0.5% of flat nodes (1731 / 370k) picked a different, equally-valid downstream receiver at a different rank count → a partition-dependent `(I − Wᵀ)` operator → the pinched near-singular regions / KSP failures that varied with the cut.
- **`self.gid` is now the partition-invariant tie-break id.** It *was* the PETSc per-partition DMPlex numbering (`l2g`), which is consistent across ranks within a run but RENUMBERED per partition (`gid[locID]` differs across np, identity only at np=1) — so every slope/flat tie-break keyed on it was technically non-invariant. `self.gid` is its ONLY use (the `mfdreceivers`/`mfdrcvrs`/flat tie-breaks + a diagnostic print), so it's now defined as **`self.locIDs.astype(int32)`** (the input-mesh index, KDTree on coords — invariant across any decomposition; the PETSc `l2g` is still built for `lgmap_col`). Impact was small for MFD (the receiver *set* is slope-determined and was already bit-invariant — only the steepest-first `[:,0]` *order* varied), but it genuinely fixed the **SFD** path (`iceplex` calls `mfdreceivers(1, …)` where the single receiver IS the tie-break) and the flat router. Verified on the 370k mesh np=1-vs-np8: mfd receiver-set flips 0, `lFill` 0, flat receivers 0.

**The fix** (`_dirFlats`, Python — no fortran recompile): replaced the `fill_rcvs` call with a vectorised receiver pick that, among strictly-closer (`ptdir(c) < ptdir(i)`) not-higher (`lFill(c) ≤ lFill(i)`) neighbours, takes the smallest `ptdir`, **ties broken by smallest `locIDs`** — identical across any decomposition. Non-tied cases unchanged. Verified locally: the 1731 receiver flips → **0**; `lFill`/`lsink`/receivers now bit-for-bit np-invariant. (This changes the tie resolution vs the old `fill_rcvs` *even at np=1* — an equal-distance alternative path to the same spill, physically equivalent — so any pre-existing bit-fingerprint at fixed np shifts; `test_parallel_correctness`/mass-conservation hold. The fortran `fill_rcvs` is now unused.)

**Related: isolated-pocket sinks + flat-BFS robustness.** `_dirFlats` also uses **stagnation detection** (stop when a pass makes no progress) instead of a fixed 100-pass cap, plus a **final halo refresh before the receiver pick** (the last `nghb_dir` updates owned nodes only; stale ghost directions otherwise made two ranks pick mutually-pointing receivers → a 2-cycle → singular block — a real partition-dependent failure, now gone). A handful of flat nodes still can't reach a spill and fall back to **self-sinks** — the pinpoint diagnostic showed these are **isolated filled pockets** (single cells / tiny clusters in non-pit terrain sharing a watershed's pitID+spill but not pit-connected to it). A sink is the physically-correct local outcome (they pond; mass stays in the pit), so this is benign. `verbose` surfaces the count + a per-node `[pitfill-diag]` dump.
