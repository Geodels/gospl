...
**Known limitation (do NOT assume it works):** fine-enriched overspill is NOT implemented — sediment overspilling a filled pit carries the routed composition, not a coarse-trapped/fine-enriched split. A conservative version needs the fine volume threaded through the iterative `_moveDownstream` cascade; an attempt was reverted because the `vSedLocal` accumulation semantics are subtle and a mismatched fine mirror leaked fine, **which the total-cumED `test_dual_mass_conservation` does NOT catch (it checks total, not per-fraction)**. See `docs/DESIGN_DUAL_LITHOLOGY.md` §8.
...
- **`mesher/unstructuredmesh.py`** — owns `dm`, every shared mesh attribute, forcing dispatch, and `destroy_DMPlex` (lines 738-799) which names every Vec/Mat by hand. Adding a new persistent Vec elsewhere requires adding it to that destroy list or it leaks (the dual-lithology `self.vSedF`/`self.vSedFLocal` are registered there).
...
| 2026-06-12 | — (on `dev`) | **Dual-lithology (coarse/fine) sediment** merged into `dev` (PRs #415 co-transport + #416 separate transport/diffusivity). Opt-in via `strata: dual`; single-fraction byte-identical. Phases 0–7: `_extraStrata` parser, `stratHf`/`phiF` state + I/O/restart, `_surfaceComposition`/`_surfaceLithoK`/`_surfaceLithoD` hooks, `erodeStrat` split, separate fine flux (`vSedF`), composition-only depth-biased lake (`_pitFineFraction`) + marine (`_marineFineFraction`) deposition, per-fraction compaction (`_depthPorosityDual`), fine-pile advection, composition-weighted hillslope + soil diffusivity. 7 dual tests incl. bitwise parity + closed-sphere per-fraction mass conservation. Full plan/limitations: `docs/DESIGN_DUAL_LITHOLOGY.md`. Known limitation: fine-enriched overspill not implemented (§8). |
