...
|---|---|---|---|
| `flow/flowplex.py` | `_solve_KSP` | `self._ksp_main` | KSP (richardson+bjacobi, `flowacc_` prefix, `pc_factor_shift_type nonzero`) |
| `flow/flowplex.py` | `_solve_KSP2` | `self._ksp_fallback` | KSP (fgmres+asm, `flowaccfb_` prefix, factor-shift) |
...
## Ice sheet (SIA + diagnostic)
Opt-in via an `ice` YAML section (`self.iceOn`). **`ice.flow_model`** selects the ice driver: **`mfd` (DEFAULT) — a cheap diagnostic (`_iceFlowMFD`)** — or `sia` (the explicit SIA thickness solve). Both feed the SAME abrasion / till / loading / output machinery (`iceUbL`→`iceAbrL`→`glacialTill`, `iceMeltL`, flexure); only the thickness/velocity *source* differs. The default is `mfd` because the SIA is stiff and over-thickens km-scale continental ice at coarse resolution; `mfd` is robust and suited to glacial-erosion morphology. (Test fixtures that exercise the SIA pin `flow_model: sia` explicitly.) Full design: `docs/DESIGN_ICE_SHEET.md`; change log: `docs/ICE_SHEET_SUMMARY.md`. Lives in `flow/iceplex.py` (`IceMesh`).

**Diagnostic (`flow_model: mfd`).** `_iceFlowMFD` routes the ELA accumulation (`mdot⁺·larea`, a volume rate) downhill through an MFD matrix (`_matrixIceFlow`, same epsfill+`mfdreceivers` machinery as water) into an **ice discharge** `iceFAL`/`iceFAG` (m³/yr, one `_solve_KSP`), smooths it, derives a **Bahr thickness** `iceHL = icewe·icewf·Q^0.3` and a **basal sliding velocity** `iceUbL` from the SIA sliding law on that thickness + bed slope (`ice_velocity`, `∝ H^(n-1)|∇s|^(n-1)`) driving `E_g=Kg|u_b|^l`. (A raw balance velocity `Q/(H·√area)` was tried first but blows up with catchment size — `Q` grows with the upstream area, `W` stays cell-scale — and spikes at flow-convergence cells, giving tens-of-km erosion/deposition in downstream sinks; the bounded sliding law fixes that. Magnitude is tuned via `Kg`.) NO dynamics solve — ~1 routing solve/step, stable + physical at any resolution (real 30 km global: 0.8 s/step, `Hmax`~1 km), where the SIA becomes stiff/over-thick for km continental ice. Config: `icedir`/`eheight`(icewe)/`fwidth`(icewf)/`melt`(meltfac). New persistent Vecs `iceFAL`/`iceFAG` + cached `iceMat` are in `destroy_DMPlex`. Output adds `iceFA`.

**Dynamics.** `iceAccumulation` runs the explicit SIA solve `_iceFlowSIA`: the goSPL step is split into substeps `H ← H + Δt_sub(ṁ − ∇·q)`, `∇·q` from the **`ice_flux_rscaled`** Fortran kernel on the total surface `s=zbed+H`. The limiter factor (kernel **`ice_flux_limiter`**, `R(k)=min(1, H_k·A_k/(Δt·outflux_k))`) caps each cell's outflux to the ice it holds; it is **halo-synced** (so a partition-boundary cell's two ranks scale the shared face identically) then each edge flux is scaled by the source cell's `R` and applied equal-and-opposite — the scheme is therefore **mass-conservative to machine precision AND `H≥0` for any substep** (serial and MPI), so the free-boundary ice margin (an obstacle problem) needs no mass-injecting post-clamp. Positivity being unconditional, `Δt_sub` is an *accuracy* knob (`sia.cfl`, default 0.5; cap `sia.max_substeps`, default 500). The substep CFL combines two rates: **flux outflux + ablation loss, capped at `1/dt`** (thin/steep/ablating cells are bounded by the limiter+clamp, so they must not drive the substep to zero — that was a real stall), **plus accumulation gain `mdot⁺/max(H, ICE_HREF)`, NOT capped** (a thin/zero accumulation-zone cell must thicken gradually so the flux can drain it as it builds — else one giant substep dumps a whole step's accumulation before any flow → **runaway km-thick ice**, the original real-model failure: 42 km in 2 steps). The melt-only post-clamp in `_iceSIAFinalize` removes over-ablation (never creates ice). **SMB accumulation controls** (`sia.accum_factor` default 1.0, `sia.accum_max` default none) scale/cap the positive SMB only (ablation untouched): `_iceSIAParams` builds `mdot = rainVal·ramp` then applies them — needed because converting full precipitation (can be several m/yr) directly to ice is unphysically high and makes km-thick ice both wrong and expensive (km ice is genuinely stiff: lower `cfl` for accuracy, costs more substeps).

> **Why explicit, not implicit.** A cached `ngmres`/Newton SNES on the backward-Euler residual `F(H)=H−Hold−Δt(ṁ−∇·q)` was the original design but **diverges (`reason −9`) on real continental runs**: the `H≥0` free boundary is an *obstacle problem* (at the margin `H=0` but the residual `≠0`), so no `F(H)=0` root-find converges there — even an exact-LU Newton step with an FD-validated analytic Jacobian fails the line search; clamping converges but injects mass. The explicit flux-limited scheme sidesteps this entirely and the CFL is benign at goSPL scales (the old "explicit is prohibitive" assumption was wrong — measured CFL is 10³–10⁴ yr, not years). Full investigation: `docs/DESIGN_ICE_SHEET.md` §9. The `ice_flux_jacobian`/`sia_diff_coeff` kernels from that work live on branch `fix/sia-solver-investigation` for any future implicit obstacle/VI solver. `_iceSIAParams` builds the ELA mass-balance ramp `ṁ = rainVal·clip((z−elaH)/(iceH−elaH),·,1)` — **precipitation-scaled** (so ablation magnitude also scales with `rainVal`; it is not a degree-day melt). `_iceSIAFinalize` clamps `H≥0`, removes ice below `max(hterm, sealevel)`, and sets `iceUbL` (basal speed, `ice_velocity` kernel), `iceAbrL` (= `Kg·|ub|^l`), `iceMeltL` (precip-scaled ablation pattern, used as the till melt-out / deposition weight), and **`iceMeltRiverL`** (the meltwater delivered to rivers, re-injected by `flowplex.flowAccumulation`). `iceMeltRiverL` is **discharge-conserving by default** (`ice.melt_conserve`, default True): `_glacialMeltwater` routes the accumulation (`mdot⁺·area`) down the ice surface and releases it where the ice melts out (`f=1` at the margin), so `Σ river-melt == Σ accumulation` (closes the glacial water budget over the long, ~steady-state goSPL step). `melt_conserve: False` reverts to the precip-scaled local ablation. **Keep `iceMeltL` (till) and `iceMeltRiverL` (river) distinct** — conflating them broke till conservation (the conserving melt-out lands at accumulation cells, not the ablation zone).
