...
- **FV stencil operators must use `_assembleDiffMatCSR`, NOT a diagonal-preallocated matrix + per-neighbour `axpy`.** This was a real **ubuntu-only `SIGABRT`**: `addprocess._buildOroMat` (orography) built `mat` via `_matrix_build_diag` (nnz=1/row, diagonal only) then `mat.axpy(1.0, tmpMat)` in a `maxnb` loop to add the off-diagonal neighbour columns — forcing `MatAXPY` to grow an under-preallocated matrix. ubuntu's PETSc build aborts (silent `SIGABRT`, no PETSc/glibc message) inside that `MatAXPY`; **macOS tolerates the reallocation**. So `test_orography_rain_shadow`'s model setup crashed deterministically on ubuntu, deep in the slow tier — surfaced first when an RC `master`/`release-candidate` push ran the full tier via `tests-pr.yml` (`tests-slow`'s slow step is gated OFF on push, so `dev` never hit it). Fix (commit `faebca3`): port `_buildOroMat` to the shared single-pass `_assembleDiffMatCSR` (nnz=`1+maxnb` preallocation, one `ADD_VALUES` `setValuesLocalCSR`, no `axpy`) — the same path advection (`_buildAdvecMat`), diffusion and flexure already use. **Do NOT reintroduce the diag+`axpy` assembly for any `(lpoints, 1+maxnb)` FV stencil operator** — it's the deprecated pattern and it aborts on ubuntu. (NB: this is NOT resource accumulation — the `tests/conftest.py` autouse `_release_petsc_after_test` teardown, added in `cf4c736` on that wrong hypothesis, made no difference to this crash; it stays only as cheap per-test GC/`garbage_cleanup` hygiene, not as the SIGABRT fix.)
