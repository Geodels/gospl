...
**Mechanism #2 of the partition-dependence (a real, small, unfixed limitation).** The cascade solves run on the **filled** topography, and the parallel depression-fill + flat-routing (`pitfilling.py`) are per-rank-local + halo-exchange algorithms whose result is *supposed* to be partition-invariant but **isn't quite** at depressions that straddle a partition cut (`spill_pts` scans only local neighbours; `label_pits`+ID-transfer; the cross-rank flat BFS). So a depression cleanly contained at one rank count gets **pinched** into a near-singular micro-region (a few nodes that can't drain) at another — at a *different* location each time, absent at some counts (e.g. seen at 144/192, not 240). The KSP then "fails" on just those nodes. It is now **bounded** (fast-fail at the caps → zeroed → finite; the discharge clamp keeps it sane) — a few zeroed nodes per affected partition, mass-conserved. **It is not a crash and not the old escalation/grind, but it is a residual correctness nick.** The proper fix is making the filled-topography flat routing globally canonical (route flats off the global spill-graph instead of the per-rank BFS) — a deep change to the parallel pit algorithm, deliberately deferred.

**Related: isolated-pocket sinks (`pitfilling._dirFlats`).** The flat-direction BFS now uses **stagnation detection** (stop when no pass makes progress) instead of a fixed 100-pass cap, plus a **final halo refresh before `fill_rcvs`** (the last `nghb_dir` updates owned nodes only; stale ghost directions otherwise let two ranks pick mutually-pointing receivers → a 2-cycle → singular block — this fixed a real partition-dependent KSP failure). A handful of flat nodes still can't reach a spill and fall back to **self-sinks** — the pinpoint diagnostic showed these are **isolated filled pockets** (single cells / tiny clusters in non-pit terrain sharing a watershed's pitID+spill but not pit-connected to it). A sink is the physically-correct local outcome (they pond; mass stays in the pit), so this is benign — same root (parallel-fill partition-sensitivity) as mechanism #2. `verbose` surfaces the count + a per-node `[pitfill-diag]` dump.
