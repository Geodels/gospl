...
class GlobalFlex(object):
    ...
    .. warning::
        ...
        This class is a simple hack to compute global flexural response based on 2D local loading changes and is likely not the best way for solving this problem. Preferred methods would consist in using spherical harmonics instead...

    This class defines global flexural responses. It allows to compute isostatic deflections of Earth's lithosphere with uniform or non-uniform flexural rigidity.

    Evolving surface loads are defined from erosion/deposition values associated to modelled surface processes.

    The function takes an initial (at time t) and final topography (at time t+Dt) (i.e. before and after erosion/deposition) and returns a corrected final topography that includes the effect of erosional/depositional unloading/loading. It uses the finite difference solutions available from gflex model (`Wickert, 2016 <https://gmd.copernicus.org/articles/9/997/2016/gmd-9-997-2016.pdf>`_) to solve the following equation:
    ...
    def _makeBnds(self, dlon=20, dlat=15):
        """
        Defines the bounds of individual tiles.

        :arg dlon: the longitudinal range of the tile in degrees
        :arg dlat: the latitudinal range of the tile in degrees
    ...
    def _shiftValues(self, ds):
        """
        Shift values by 180 degrees to remove longitudinal boundary effect.

        :arg ds: xarray dataset to shift
    ...
    def _interpUTM(self, ds):
        """
        Interpolates the projected UTM values on a regular grid to perform flexural isostasy.

        .. note::

                The interpolation is performed using xarray built-in function `interp` using a bicubic method.

        :arg ds: xarray dataset to shift

        :return: the interpolated xarray dataset
        """
    ...
    def _makeUTM(self, ds, lonlat_bnds, dll=10):
        """
        Clips the global lon/lat grid into tiles and performs a reprojection in UTM coordinate.

        :arg ds: xarray dataset in lon/lat coordinates
        :arg lonlat_bnds: the tile lon/lat coordinates
        :arg dll: the additional lon/lat degrees added to the tile

        :return: interp, the interpolated xarray dataset
        """
    ...
    def _makeLonLat(self, utmds, bnds, res):
        """
        Transforms the UTM grid back to lon/lat coordinates.

        :arg utmds: xarray UTM dataset
        :arg bnds: the tile lon/lat coordinates
        :arg res: the lon/lat resolution to use.

        :return: map, the flexural response in lon/lat coordinate for each tile
        """
    ...
    def _interpLonLat(self, ds, bnds, res):
        ...
        .. note::
                ...
                The interpolation is performed using xarray built-in function `interp` using the nearest method.

        :arg ds: xarray dataset
        :arg bnds: the tile lon/lat coordinates
        :arg res: the lon/lat resolution to use.
    ...
    def _interpLonLat2(self, ds, bnds, res):
        """
        Transforms the UTM grid back to lon/lat coordinates for the North and South poles.
    ...
    def _getFlex(self, ds, fds, fdse):
        """
        Get flexural response values.

        :arg ds: initial xarray dataset
        :arg fds: flexural xarray dataset
        :arg fdse: flexural xarray dataset from shifted data

        :return: flexural response dataset
        """

        nfds = xr.Dataset({
            'empty': xr.DataArray(np.nan,
                                  coords=dict(latitude=ds.latitude.values,
    ...
    def _lon_lat_box(self, lon_bounds, lat_bounds, refinement=2):
        """
        Define lon/lat tiles bounds for the North and South hemispheres.

        :arg lon_bounds: longitudinal range
        :arg lat_bounds: latitudinal range
        :arg refinement: tile refinement
    ...
    def _getSouthPole(self, ds):
        """
        Get South hemisphere dataset.

        :arg ds: xarray dataset

        :return: xarray UTM dataset
        """
    ...
    def _cmptFlex(self, xrutm, regrid=1, shifted=False):
        """
        Computes flexural isostasy on each UTM tile.  It uses the finite difference solutions available from gflex model (`Wickert, 2016 <https://gmd.copernicus.org/articles/9/997/2016/gmd-9-997-2016.pdf>`_).

        :arg xrutm: xarray UTM dataset
        :arg regrid: regriding value
        :arg shifted: boolean to indicate whether this is the shifted or not values

        :return: xrutm, the flexural response as a xarray UTM dataset
        """

        if regrid > 1:
            xcoord = xrutm.x.values.copy()
            ycoord = xrutm.y.values.copy()
            xrutm = xrutm.coarsen(y=regrid, x=regrid, boundary='pad').mean()
    ...
    def _getTiles(self, val_ds):
        """
        Splits the global grids into tiles and projects in Cartesian coordinates.

        :arg val_ds: xarray dataset

        :return: flexGrids, flexNGrid, flexSGrid, flexEdges: the flexural response as a series of xarray datasets.
        """
        ...
        flexGrids, flexNGrid, flexSGrid, flexEdges = self._flexTiles(listbnds, list_clip_utm,
                                                                     north_utm, south_utm)

        return flexGrids, flexNGrid, flexSGrid, flexEdges

    def _flexTiles(self, listbnds, list_clip_utm, north_utm, south_utm):
        """
        Compute flexural isostasy locally and reproject to lon/lat.

        :arg listbnds: list of lon/lat bounds
        :arg list_clip_utm: list of UTM bounds
        :arg north_utm: northern UTM bounds
        :arg south_utm: southern UTM bounds

        :return: flexGrids, flexNGrid, flexSGrid, flexEdges: the flexural response as a series of xarray datasets.
        """

        flexgrd = 2  # Hard-coded values could be changed in the future
        ...
        MPIcomm.Barrier()
        if MPIrank == 0 and self.verbose:
            print("--- Perform flexural isostasy (%0.02f seconds)" % (process_time() - tstep), flush=True, )

        return flexGrids, flexNGrid, flexSGrid, flexEdges

    def _mergeTile(self, flexds, flexdse):
        """
        Merge all tiles to build the global flexural isostasy reponse.

        :arg flexds: flexural xarray dataset
        :arg flexdse: flexural xarray dataset from the shifted values

        :return: flexds, flexdse
        """
    ...
    def _runFlexure(self, val_ds):
        ...
        flexGrids, flexNGrid, flexSGrid, flexEdges = self._getTiles(val_ds)
        ...
        if MPIrank == 0:
            ...
            flexds, flexdse = self._mergeTile(flexds, flexdse)
    ...
    def globalFlexIso(self):
        """
        Main entry point to the global flexural isostasy computation.
        """
        ...
        ds = xr.Dataset({'elevation': xr.DataArray(elev, coords=dict(latitude=self.lati, longitude=self.long), dims=("latitude", "longitude")), 'erodep': xr.DataArray(erodep, coords=dict(latitude=self.lati, longitude=self.long), dims=("latitude", "longitude")), })
