...
Adapted from benchmarks/knickpoint/test-knickpoint.py for the pytest
harness. The nine core helpers (StepData, loadTopology, loadStepData,
getStepData, getStream, detectKnickpoint, evaluateKnickpoint,
printResults, writeMarkdown, writeFigures) are preserved verbatim from
the original. Only the entry point is replaced with a pytest function.
...
@pytest.mark.slow
def test_knickpoint_propagation(knickpoint_tmp_path):
    """
    Knickpoint propagation benchmark.

    Validates: SPL wave celerity c = K·A^m, position integral,
    knickpoint height conservation, upstream profile preservation.
    Analytical basis: Royden & Perron (2013), Tucker & Whipple (2002).
    Pass criteria: 4/4 sub-tests must pass.
    See AGENTS.md: Analytical benchmark suite.
    """
    time_start = time.time()

    # ------------------------------------------------------------------
    # Phase 1 — Steady-state run (try/finally per AGENTS.md KSP contract).
    # ------------------------------------------------------------------
    print("=" * 57)
    ...
    try:
        ...
        model.destroy()
    print(f"Phase 1 complete  ({time.time()-time_start:.1f} s)")

    # ------------------------------------------------------------------
    # Phase 1 post-processing — topology + steady-state outputs.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Build the dropped-base-level mesh from final SS elevation.
    # DROP_MESH path is relative to cwd = knickpoint_tmp_path.
    # ------------------------------------------------------------------
    data   = np.load(MESH_FILE)
    v, c_mesh, t_mesh = data["v"], data["c"], data["t"]
    data.close()

    # Outlet nodes (x = 0 on the western edge) get pinned to -dz_drop.
    ...
    print(f"\nDrop mesh saved → {DROP_MESH}  "
          f"({len(ids)} outlet nodes dropped by {dz_drop:.0f} m)")

    # ------------------------------------------------------------------
    # Phase 2 — Base-level drop run.
    # ------------------------------------------------------------------
    print("\n" + "=" * 57)
    ...
    try:
        ...
        model.destroy()
    print(f"Phase 2 complete  ({time.time()-time_start:.1f} s)")

    # ------------------------------------------------------------------
    # Load drop-run outputs and post-process.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Extract main stem from final SS step (fixed geometry reference).
    # ------------------------------------------------------------------
    x = coords[:, 0]
    y = coords[:, 1]
    ...
    # ------------------------------------------------------------------
    # Build per-step stream arrays and detect knickpoint.
    # ------------------------------------------------------------------
    stream_elev     = []
    stream_elev_pre = []
    stream_dist     = []
    ...
    # ------------------------------------------------------------------
    # Assert per A6 spec — evaluateKnickpoint already exposes overall_pass.
    # ------------------------------------------------------------------
    assert results['overall_pass'], (
        f"Knickpoint benchmark failed: "
        f"{results['n_pass']}/4 tests passed\n"
        f"  Celerity error (med) : {100*results['celerity_err_med']:.1f}%  "
        f"(< {100*CELERITY_THRESHOLD:.0f}%)\n"
        f"  Position R²          : {results['position_r2']:.4f}  "
        f"(> {R2_THRESHOLD:.2f})\n"
        f"  Height conservation  : Δz = {results['height_median']:.1f} m  "
        f"({100*results['height_err']:.1f}% error, < {100*CONSERVATION_TOL:.0f}%)\n"
        f"  Upstream preservation: {results['upstream_dev']:.3f} m  "
        f"(< {100*UPSTREAM_TOL:.0f}% of drop)"
    )
