...
| **1** | SIA thickness evolution: import `ice_flux`; **implicit (semi-implicit SNES, reusing the `_hillSlopeNL`/`fctcoeff` pattern)** solve of `∂H/∂t = ṁ − ∇·q` producing `iceH`; ELA SMB as source. Picard/line-search fallback for stiffness. **Validate against the Halfar/Bueler analytical SIA dome** (transient + steady). Optional explicit subcycled path kept for small-case validation only. | `iceplex.py`, Fortran import, `destroy_DMPlex` | **high** |
