...
| G5 | **DONE.** Solute-source provenance (opt-in, needs in-model `provenance:`): by linearity the solute is transported **per source-rock class** (same operator, class-restricted RHS `_soluteSolveRHS`), so the downstream crust is attributed to the upgradient region where its solute dissolved. Per-node `gwCrustProv` (crust by source class) → dominant-source field `gwCrustSource`, output `crust_source`. Unblocks `DESIGN_WATERTABLE_DURICRUST.md` §11. **As built: per-node** attribution (the per-stratigraphic-layer `stratDuri`-riding record is a further refinement). | `test_geochem_provenance` (both sources contribute, per-class sums to total, `crust_source` = argmax, resolves to both) |
