...
subroutine ice_flux_limiter(nb, hice, zbed, ad, as, glen, dt, rfac)
!*****************************************************************************
! Per-cell flux-limiter factor for the mass-conserving explicit SIA step. The
! factor caps a cell's TOTAL outflux to the ice it actually holds over the step:
!
!   rfac(k) = min(1, hice(k)*FVarea(k) / (dt*outflux(k)))    (1 if no outflux)
!
! with outflux(k) the sum of the outgoing (vol>0) edge fluxes (same edge fluxes
! as ice_flux). Computed for ALL local cells, but only OWNED cells have the full
! neighbour stencil, so the caller MUST halo-sync rfac (so ghosts carry their
! owner's factor) before ice_flux_rscaled — otherwise conservation breaks at a
! partition boundary when a boundary cell is actually limited (rfac<1).
    ...
    double precision, intent(out) :: rfac(nb)

    integer :: k, p, i
    double precision :: hs, midh, nhs, s, ud, us, vol, outflux, avail
      ...
      if(FVarea(k)>0)then
        ...
        do p = 1, FVnNb(k)
          if(FVvDist(k,p)>0.)then
            ...
            us = as*midh**(glen-1.)*abs(s)**(glen-1.)*s
            vol = (ud+us)*midh*FVvDist(k,p)
            if(vol>0.) outflux = outflux + vol
          endif
        enddo
        if(outflux>0.)then
          avail = hice(k)*FVarea(k)
          if(dt*outflux > avail) rfac(k) = avail/(dt*outflux)
        endif
      endif
    enddo
...
end subroutine ice_flux_limiter

subroutine ice_flux_rscaled(nb, hice, zbed, ad, as, glen, rfac, val)
!*****************************************************************************
! Mass-conserving, positivity-preserving SIA flux divergence for an EXPLICIT
! step  H <- H + dt*(mdot - val). Identical edge fluxes to ice_flux, but every
! edge flux is scaled by the SOURCE cell's limiter factor `rfac` (rfac(k) when
! the flux leaves k, else rfac(i) for the up-edge neighbour i), where rfac comes
! from ice_flux_limiter and has been halo-synced so both cells of a shared face
! (even across a partition boundary) use the SAME factor. The scaled flux is
! therefore applied equal-and-opposite across each face -> ice volume conserved
! exactly; and no cell exports more ice than it holds -> H stays >= 0 for ANY
! dt. The substep size is then an accuracy choice, not a stability constraint.
    ...
    double precision, intent(in) :: as
    double precision, intent(in) :: glen
    double precision, intent(in) :: rfac(nb)
    double precision, intent(out) :: val(nb)

    integer :: k, p, i
    double precision :: hs, midh, nhs, s, ud, us, vol, rk
...
end subroutine ice_flux_rscaled
