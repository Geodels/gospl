...
class PFit(object):
    ...
    def __init__(self):
        ...
        if abs(len(gZ) - self.gpoints) > 0:
            print(
                "Forcing paleotopography file size differs from loaded one: %s",
                self.paleoTopo,
            )
        ...
        icoords = loadData["v"]
        icoords[0, :] = icoords[0, :] * 360.0 / 3601.0 - 180.0
        icoords[1, :] = icoords[1, :] * 180.0 / 1801.0 - 90.0

        # Target grid to interpolate to
        ilon = np.arange(-180.0, 180.0, 1.0)
        ilat = np.arange(-90.0, 90.0, 1.0)
        ...
        self.ll_wght = np.divide(
            np.ones(dist.shape), dist ** 2, out=np.zeros_like(dist), where=dist != 0
        )
        self.ll_oid = np.where(dist[:, 0] == 0)[0]

        # Map paleotopography onto the regular grid
        self.paleoZ = np.sum(self.ll_wght * gZ[self.ll_id], axis=1) / np.sum(
            self.ll_wght, axis=1
        )
        ...
        self.paleoLand = np.where(self.paleoZ >= 0.0)
        self.paleoOcean = np.where(self.paleoZ <= -1000.0)
        self.paleoShelf = np.where(
            np.logical_and(self.paleoZ < 0.0, self.paleoZ > -1000.0)
        )
    ...
    def comparePaleomaps(self, verbose=False, disk=False):
        ...
        if self.runNb == 0:
            ...
            if (
                self.accuracy[-2].values[0][4] >= self.cvglimit
                and self.runNb <= self.paleostep
            ):
    ...
    def backwardAdvection(self, verbose=False):
        ...
        if MPIrank == 0:
            ...
            for k in range(nbdisp):
                ...
                weights = np.divide(
                    np.ones(distances.shape),
                    distances ** 2,
                    out=np.zeros_like(distances),
                    where=distances != 0,
                )
                onIDs = np.where(distances[:, 0] == 0)[0]
                nzdisp = np.sum(weights * self.updated_diff[indices], axis=1) / np.sum(
                    weights, axis=1
                )
                if len(onIDs) > 0:
                ...
                if p == nbdisp - 1 and self.force_diff is not None:
                    np.savez_compressed(
                        outfile[k], z=nzdisp + self.force_diff * self.simtime / dt
                    )
                else:
                ...
                onIDs = np.where(distances[:, 0] == 0)[0]
                nadvect[:, 0] = np.sum(weights * ndisps[indices, 0], axis=1) / np.sum(
                    weights, axis=1
                )
                nadvect[:, 1] = np.sum(weights * ndisps[indices, 1], axis=1) / np.sum(
                    weights, axis=1
                )
                nadvect[:, 2] = np.sum(weights * ndisps[indices, 2], axis=1) / np.sum(
                    weights, axis=1
                )
    ...
    def _computeNorms(self, map1, map2):
        """
        Manhattan distance is a distance metric between two points in a N dimensional
        vector space. It is the sum of the lengths of the projections of the line
        segment between the points onto the coordinate axes. In simple terms, it is
        the sum of absolute difference between the measures in all dimensions of
        two points.
    ...
    def _accuracyScores(self):
        ...
        Lobs = obs.copy()
        Lsim = sim.copy()
        Lobs[self.paleoZ < 0] = 0.0
        Lsim[self.paleoZ < 0] = 0.0
        ...
        Sobs = obs.copy()
        Ssim = sim.copy()
        Sobs[self.paleoZ > 0] = 0.0
        Ssim[self.paleoZ > 0] = 0.0
        Sobs[self.paleoZ < -1000.0] = 0.0
        Ssim[self.paleoZ < -1000.0] = 0.0
        ...
        Oobs = obs.copy()
        Osim = sim.copy()
        Lobs[self.paleoZ >= -1000.0] = 0.0
        Lsim[self.paleoZ >= -1000.0] = 0.0
        ...
        LCC.append(
            self._concordance_correlation_coefficient(
                ocean_obs.ravel(), ocean_sim.ravel()
            )
        )
