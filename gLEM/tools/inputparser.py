        ...
        try:
            ...
            for k in range(len(tecSort)):
                ...
                try:
                    ...
                    raise ValueError(
                        "Tectonic event {} has no parameter start".format(k)
                    )
                try:
                ...
                if tMap == "empty" and zMap == "empty":
                    ...
                    raise ValueError(
                        "Tectonic event {} has no tectonic map (map).".format(k)
                    )
                ...
                if tStep is not None:
                    if tEnd is not None:
                        ...
                        while tectime < tEnd:
                            tmpTec = []
                            tmpTec.insert(
                                0, {"start": tectime, "tMap": tMap, "zMap": zMap}
                            )
                            tecdata = pd.concat(
                                [
                                    tecdata,
                                    pd.DataFrame(
                                        tmpTec, columns=["start", "tMap", "zMap"]
                                    ),
                                ],
                                ignore_index=True,
                            )
        ...
        try:
            ...
            for k in range(len(flexSort)):
                ...
                try:
                    ...
                    raise ValueError(
                        "elastic thickness {} has no parameter time".format(k)
                    )
                try:
